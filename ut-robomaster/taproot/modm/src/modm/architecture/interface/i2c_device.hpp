 ...
 *
 * This class provides functions for configuring the adapter and peripheral and starting
 * transactions. It also provides a basic `ping()` resumable function, which checks if the device
 ...
 *
 * You can choose to use your own custom I2c Transaction Adapter, by providing a class subclassed
 * from an existing I2cAdapter or which implements the I2cTransaction interface manually.
 *
...
public:
    ...
    ///	@param	address	the slave address not yet shifted left (address < 128).
    void setAddress(uint8_t address) { transaction.setAddress(address); }

    /// Attaches a configuration handler, which is called before a transaction,
    /// whenever the configuration has to be changed.
    void inline attachConfigurationHandler(I2c::ConfigurationHandler handler)
    {
...
protected:
    ...
    {
        return (
            transaction.configureWriteRead(writeBuffer, writeSize, readBuffer, readSize) and
            startTransaction());
    }

    /// Configures the transaction with a write operation and starts it.
    bool inline startWrite(const uint8_t *buffer, std::size_t size)
    {
        return (transaction.configureWrite(buffer, size) and startTransaction());
    }

    /// Configures the transaction with a read operation and starts it.
    bool inline startRead(uint8_t *buffer, std::size_t size)
    {
        return (transaction.configureRead(buffer, size) and startTransaction());
    }

protected:
    /// Starts the transaction with our own transaction object.
    bool inline startTransaction() { return startTransaction(&this->transaction); }

    /// Starts the transaction with a seperate transaction object.
    bool inline startTransaction(modm::I2cTransaction *transaction)
    {
        return I2cMaster::start(transaction, configuration);
    }

    /// @returns `true` when transaction is busy.
    bool inline isTransactionRunning() { return transaction.isBusy(); }

    /// @returns `true` when transaction did not return an error.
    bool inline wasTransactionSuccessful()
    {
